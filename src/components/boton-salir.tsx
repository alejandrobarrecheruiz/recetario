 ...
 */
export function BotonSalir({ className, disabled = false }: { className?: string; disabled?: boolean }) {
  ...
  return (
    <span><button
      ...
      onClick={async () => {
        ...
        if (resultado.error) throw new Error("salida");
        router.replace("/");
    ...
    {error && <span role="alert" className="block text-sm text-acento">No se pudo cerrar la sesión. Vuelve a intentarlo.</span>}</span>
  );
}
