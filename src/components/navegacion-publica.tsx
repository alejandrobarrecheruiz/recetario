...
import { useRutaCatalogo } from "@/components/retorno-catalogo";

export function NavegacionPublica() {
  const ruta = usePathname();
  const ultimaRutaCatalogo = useRutaCatalogo();
  const volviendo = ruta.startsWith("/recetas/");
  const etiquetaCatalogo = volviendo ? "Volver al catálogo" : "Todas las recetas";
  return (
    <nav aria-label="Principal" className="navegacion-publica">
      <Link href={volviendo && ultimaRutaCatalogo ? ultimaRutaCatalogo : "/recetas"} scroll={!volviendo || !ultimaRutaCatalogo} aria-label={etiquetaCatalogo} aria-current={ruta === "/recetas" ? "page" : undefined} className="icono-cabecera">
        ...
        <span aria-hidden="true" className="ayuda-cabecera">{etiquetaCatalogo}</span>
      </Link>
